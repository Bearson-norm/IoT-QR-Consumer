...
function parseEmployeeData(data) {
  ...
  for (const line of lines) {
    ...
    if (parts.length >= 2) {
      ...
      // Only name (no employee_id) - skip or use previous ID
      // For now, we'll skip entries without employee_id
...
function importEmployees() {
  ...
  const stmt = db.prepare('INSERT OR IGNORE INTO employee_data (employee_id, name) VALUES (?, ?)');
  ...
  employees.forEach((emp, index) => {
    stmt.run(emp.employee_id, emp.name, function(err) {
      if (err) {
